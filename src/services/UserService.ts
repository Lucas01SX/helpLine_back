...
export class UserService {
    ...
    public static async usuariosLogados() :Promise<any> {
        try {
            const result = await pool.query(`select b.login, b.nome, b.codfuncao, e.de_funcao, string_agg(d.segmento, ',') as segmento, string_agg(c.fila, ',') as fila, string_agg(c.mcdu, ',') as mcdu, max(a.hr_login) as hr_login, max(f.hora_fim_suporte) as hora_fim_suporte from suporte.tb_login_logoff_suporte a join suporte.tb_login_suporte b on a.pk_id_usuario = b.id_usuario join suporte.tb_skills_staff c on b.matricula = c.matricula::int join trafego.tb_anexo1g d on c.mcdu::int = d.mcdu join trafego.tb_funcao e on e.co_funcao::int = b.codfuncao left join suporte.tb_chamado_suporte f on a.pk_id_usuario = f.pk_id_prestador_suporte and a.dt_login = f.dt_inicio_suporte where a.dt_login = current_date and a.status = 1 and c.excluida = false group by b.login, b.nome, e.de_funcao, b.codfuncao order by hr_login, hora_fim_suporte desc`);
            return result.rows;
        } catch (e) {
